      ...
      <div className="mt-3 flex flex-wrap gap-2">
        <form action={formAction}>
          ...
          <button type="submit" disabled={pending} aria-busy={pending} className="ui-btn-primary">
            {pending ? <BusyLabel>{t("common.sending")}</BusyLabel> : t("booking.acceptSwap")}
          </button>
        </form>
        <form action={formAction}>
          <input type="hidden" name="application_id" value={applicationId} />
          <input type="hidden" name="decision" value="reject" />
          <button type="submit" disabled={pending} aria-busy={pending} className="ui-btn-secondary">
            {pending ? <BusyLabel>{t("common.sending")}</BusyLabel> : t("booking.rejectSwap")}
          </button>
        </form>
      </div>
    </div>
